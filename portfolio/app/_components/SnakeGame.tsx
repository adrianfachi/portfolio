		...
		<div className="hidden items-center justify-center bg-gradient-to-br from-[#00d5c082] to-[#43D9AD13] lg:flex rounded-2xl m-4">
			<div className="flex space-x-6 p-6 rounded-2xl shadow-xl">
				<div className="bg-[#0a1a2a] rounded-xl shadow-lg relative">
					{!started ? (
						<button onClick={resetGame} className="absolute top-2/3 left-1/2 transform -translate-x-1/2 bg-orange rounded-[8px] px-4 text-black flex items-center cursor-pointer">
							começar
						</button>
					) : gameOver ? (
						<div className="absolute top-2/3 w-full text-xl flex flex-col items-center">
							<span className="bg-[#011627] w-full text-center text-green">GAME OVER!</span>
                            <span className="text-base text-primary text-[.6rem] cursor-pointer" onClick={resetGame}>Tente novamente</span>
						</div>
					) : null}
							...
							return (
								<div
									...
									className={`h-[15px] w-[15px] ${
										isHead && !gameOver ? `bg-[#43D9AD] shadow-[0_0_8px_#22d3ee] ${
											direction === "UP" ? "rounded-t-2xl"
											:direction === "LEFT" ? "rounded-l-2xl"
											:direction === "DOWN" ? "rounded-b-2xl"
											: "rounded-r-2xl"  
										}`
										:isSnake
				...
				<div className="text-gray-200 font-mono space-y-6">
					<div>
						...
						<div className="grid grid-cols-3 gap-2 mt-3">
							<div />
							<div className="p-2 bg-[#1b2c3c] rounded-md"><IoMdArrowDropup/></div>
							<div />
							<div className="p-2 bg-[#1b2c3c] rounded-md"><IoMdArrowDropleft/></div>
							<div className="p-2 bg-[#1b2c3c] rounded-md"><IoMdArrowDropdown/></div>
							<div className="p-2 bg-[#1b2c3c] rounded-md"><IoMdArrowDropright/></div>
						</div>
					</div>
